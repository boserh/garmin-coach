...
(multi-file, non-blocking upload): `_process_checkup_upload_job` (own DB session,
broadcasts before/after, never raises), `_broadcast_job` (fan-out to open /checkups/ws
tabs, drops dead connections) and `_prune_upload_jobs` (per-process TTL sweep)."""
...
def _job(**kw):
    job = checkups_router.UploadJob(
        ...
        filename=kw.pop("filename", "lab.jpg"), **kw,
    )
...
async def test_process_checkup_upload_job_success(monkeypatch):
    job = _job(id="ocr-ok")
    fake_row = type("Row", (), {"id": 42})()
    monkeypatch.setattr(checkups_router, "run_checkup_ocr", AsyncMock(return_value=fake_row))
    try:
        await checkups_router._process_checkup_upload_job(
            job.id, b"bytes", "image/jpeg", "2026-07-20", "k")
        assert job.status == "done"
        assert job.checkup_id == 42
    finally:
...
async def test_process_checkup_upload_job_analyst_error(monkeypatch):
    job = _job(id="ocr-err")
    monkeypatch.setattr(
        checkups_router, "run_checkup_ocr", AsyncMock(side_effect=AnalystError("боом")))
    try:
        await checkups_router._process_checkup_upload_job(
            job.id, b"bytes", "image/jpeg", "2026-07-20", "k")
...
async def test_process_checkup_upload_job_unexpected_crash_is_caught(monkeypatch):
    ...
    monkeypatch.setattr(
        checkups_router, "run_checkup_ocr", AsyncMock(side_effect=RuntimeError("boom")))
    try:
        await checkups_router._process_checkup_upload_job(
            job.id, b"bytes", "image/jpeg", "2026-07-20", "k")
        assert job.status == "error"
...
async def test_process_checkup_upload_job_broadcasts_status_over_websocket(monkeypatch):
    ...
    assert sent[-1]["checkup_id"] == 7
    assert sent[-1]["job_id"] == "ocr-ws"


async def test_broadcast_job_drops_dead_connections():
    job = checkups_router.UploadJob(id="jobX", user_id=U1, filename="x.jpg")
...
async def test_broadcast_job_is_noop_with_no_listeners():
    job = checkups_router.UploadJob(id="jobY", user_id=U1, filename="y.jpg")
    checkups_router._ws_by_user.pop(U1, None)

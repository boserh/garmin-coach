...
`scripts/ops01_recon_gconn.py` — standalone (нуль імпортів з `app`), ганяти в
**окремому одноразовому venv** з найновішим `python-garminconnect`:
...
каже про Cloudflare-еру auth-движок. Додатковий факт: проєктний venv — Python
3.9; якщо свіжий `python-garminconnect` вимагає 3.10+, міграція тягне за собою
і апгрейд Python на проді (перевірити при прогоні, вписати сюди).
...
1. **Залежності**: розпінити `garminconnect` → найновіший (curl_cffi
   ...
   приведе свою сумісну версію як транзитивну залежність). Якщо треба
   Python 3.10+ — спершу апгрейд venv/Pi.

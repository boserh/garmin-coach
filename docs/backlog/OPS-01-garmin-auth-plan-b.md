...
і має **`Requires-Python >= 3.12`**. Проєктний venv — Python 3.9, Pi OS
bookworm з коробки — 3.11: тобто міграція тягне за собою апгрейд Python на
проді (uv/pyenv або новіша OS) — перевірити на Pi, чи ставиться 3.12+ і чи є
aarch64-wheel для `curl_cffi` (якщо pip почне компілювати — вписати сюди).
...
1. **Залежності**: розпінити `garminconnect` → найновіший (curl_cffi
   ...
   garminconnect (0.3.6) вимагає **Python ≥ 3.12** — спершу апгрейд
   venv/Pi (зараз: проєктний venv 3.9, Pi OS bookworm 3.11).

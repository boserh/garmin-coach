...
| [PERF-03](PERF-03-postgres-and-indexes.md) | Postgres перед мультиюзером + індекси · ❄️ **frozen** (Postgres-частина): прив'язаний до `/register` — той самий garth-ризик, що EP-06. **Аудит індексів зроблено** окремо (композитні `user_id`+дата) — див. Done | M | — |

## Безпека й операції (аудит коду 2026-07)

Обидві сторі цього розділу зроблено (2026-07) — див. Done (SEC-01, OPS-02).
Лишається одноразова операційна частина OPS-02 **на Pi**: увімкнути systemd-таймер
та задати реальний off-SD `--rsync-dest`.
